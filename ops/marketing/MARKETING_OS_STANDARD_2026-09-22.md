...
Current S.O.S. Instagram is **@SUPERHERO.ONSTANDBY**. This identity is locked as the canonical S.O.S. publishing route. All prior identities associated with the underlying account are retired from current operations and may appear only in immutable historical evidence. They may not be used for publishing, scheduling, tagging, routing, reporting, engagement, DMs, comments, or approval decisions.

The deployed social-publish-executor v9 introduced a pre-container live-username mismatch block. That change is not a certification of every publishing path or every possible failure case. Approval must ALSO bind the intended S.O.S. entity and asset to @SUPERHERO.ONSTANDBY. A retired identity or historical mapping can never make content valid for current S.O.S. publishing.
...
## Email and Shopify
**GHL is the default outbound system for marketing email, newsletters, nurture, provider recruitment, customer lifecycle, operational follow-up, and SMS.** Every entity must use its own GHL location/subaccount, sender identity, workflows, audience, suppression state, pipeline, and reporting. No brand may borrow another brand's location, sender, workflow, audience, or pipeline.

Gmail is exception-only for VC/investor outreach, founder-personal correspondence, or another explicitly approved special case. Marketing may never automatically fall back from GHL to Gmail. A ChatGPT-facing HighLevel connector IAM error is an admin-surface issue, not proof that the Supabase-backed GHL runtime is unavailable.
